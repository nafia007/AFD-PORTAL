...
const FilmmakerShowcase = () => {
  ...
  const fetchShowcaseProfiles = async () => {
    try {
      // First get the filmmaker_profiles with show_in_showcase = true
      ...
      // Then get the extended profiles
      ...
      // Join the data manually and transform
      const transformedData = basicProfiles?.map(profile => {
        const extended = extendedProfiles?.find(ext => ext.id === profile.id);
        return {
          ...profile,
          name: extended?.name || null,
          role: extended?.role || null,
          experience: extended?.experience || null,
